...
type Rule = { event: string; scope: string; targetId: string; enabled: boolean; audience: string; minutesBefore: number };
...
export default function NotificationSettings() {
  ...
  const [users,setUsers] = useState<Array<{employeeId:string;firstName:string;lastName:string}>>([]);
  ...
  useEffect(()=>{setDrafts({});setSaved('');},[scope,targetId,data]);
  function ruleFor(event:string):Rule {
    return drafts[event] ?? data?.rules.find(rule=>rule.event===event && rule.scope===scope && rule.targetId===targetId)
      ?? {...(data?.rules.find(rule=>rule.event===event && rule.scope==='organization') ?? {enabled:true,audience:'original',minutesBefore:15}),event,scope,targetId};
  }
  async function save(event:string,remove=false) {
    if(!targetId)return;setBusy(true);setError('');setSaved('');
    try {await apiFetch('/admin/push',{method:remove?'DELETE':'PUT',body:JSON.stringify(ruleFor(event))});const settings=await apiFetch<Settings>('/admin/push');setData(settings);setSaved(remove?'Override removed. The inherited setting now applies.':'Notification setting saved.');}
