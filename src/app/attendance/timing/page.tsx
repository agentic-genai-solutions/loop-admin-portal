...
export default function AttendanceTimingPage() {
  ...
  const save = async () => {
    ...
    try { new Intl.DateTimeFormat('en', { timeZone: form.timeZone }).format(); } catch { setError('Enter a valid timezone, for example Asia/Kolkata.'); return; }
  ...
  const name = (policy: Policy) => policy.scope === 'enterprise' ? 'Entire organization' : policy.scope === 'store' ? stores.find(store => store.id === policy.targetId)?.name || policy.targetId : employees.find(person => person.id === policy.targetId)?.name || policy.targetId;
  return <WorkforceWorkspace actions={<button className={styles.secondary} disabled={loading || saving} onClick={load}>Refresh</button>}>

    {error && <p role="alert" className={styles.error}>{error}</p>}
    {notice && <p role="status" className={styles.notice}>{notice}</p>}
    <form className={styles.panel} onSubmit={event => { event.preventDefault(); void save(); }}>
      <div className={styles.panelHeader}><div><h2>Attendance timing</h2><p>Set when employees can clock in and out. Employee rules take priority over store and organization rules.</p></div></div>
      <fieldset disabled={loading || saving || loadFailed} className={workforce.fieldset}>
        <div className={workforce.grid}>
          <label>Applies to<select value={form.scope} onChange={event => { setStoreFilter(''); selectTarget(event.target.value as Policy['scope'], event.target.value === 'enterprise' ? 'all' : ''); }}><option value="enterprise">Entire organization</option><option value="store">Store</option><option value="employee">Employee</option></select></label>
        ...
        <section className={workforce.section}><h3>Allowed clock-in and clock-out windows</h3><div className={workforce.grid}>{fields.map(([key,label,hint,max]) => <label key={key}>{label}<input type="number" required min={0} max={max} step={1} value={form[key]} onChange={event => setForm({...form, [key]: event.target.value === '' ? '' : Number(event.target.value)})} /><small>{hint}</small></label>)}</div></section>
        ...
        <div className="timing-actions"><label><input type="checkbox" checked={form.enabled} onChange={event => setForm({...form, enabled: event.target.checked})} /> Enable this rule</label><button className={styles.primary} type="submit">{saving ? 'Saving…' : 'Save rule'}</button></div>
      </fieldset>
    </form>
    <section className={styles.panel}><h2 style={{fontSize: 18, marginTop: 0}}>Configured rules <span className="badge info">{policies.length}</span></h2>{loading ? <p role="status">Loading…</p> : !policies.length ? <p className="timing-hint">No timing rules configured yet.</p> : <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Target</th><th>Scope</th><th>Basis</th><th>Timezone</th><th>Status</th><th /></tr></thead><tbody>{policies.map(policy => <tr key={`${policy.scope}-${policy.targetId}`}><td>{name(policy)}</td><td>{policy.scope}</td><td>{policy.mode === 'schedule' ? 'Assigned shift' : `${policy.startTime}–${policy.endTime}`}</td><td>{policy.timeZone}</td><td><span className={`badge ${policy.enabled ? 'success' : 'info'}`}>{policy.enabled ? 'Active' : 'Inactive'}</span></td><td><button type="button" className={styles.secondary} disabled={saving} onClick={() => { setForm(policy); setStoreFilter(''); window.scrollTo({top: 0, behavior: 'smooth'}); }}>Edit</button></td></tr>)}</tbody></table></div>}</section>
  </WorkforceWorkspace>;
}
