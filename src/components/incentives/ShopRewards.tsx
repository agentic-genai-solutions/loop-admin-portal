  ...
  return <section className={styles.panel}>
    ...
    {loading ? <TableSkeleton columns={5} rows={3} /> : error ? <EmptyState variant="error" onRetry={load} /> : !groups.length ? <div className={styles.empty}><span className={styles.emptyIcon}>🏆</span><h3>No {status.toLowerCase()} rewards found</h3><p>{search || shop !== 'all' ? 'Try another shop or clear your search.' : status === 'Pending' ? 'You’re all caught up.' : 'Rewards will appear here when their status changes.'}</p></div> : groups.map((group) => {
      ...
      return <section key={group.id} className={styles.rewardGroup}>
        <div className={styles.panelHeader}><div><h3>{group.name}</h3><p>{group.rows.length} rewards · {new Set(group.rows.map((reward) => reward.employeeId)).size} employees</p></div><strong>{Object.entries(totals).map(([currency, amount]) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: currency || 'INR' }).format(amount)).join(' · ')}</strong></div>
        <div className={styles.tableWrap}><table className="table"><thead><tr><th>Employee</th><th>Program</th><th>Reward</th><th>Period</th><th>{status === 'Pending' ? 'Approval' : 'Notification'}</th></tr></thead><tbody>{group.rows.map((reward) => <tr key={reward.id}><td><strong>{reward.employee}</strong><small className={styles.cellSub}>{reward.employeeId}</small></td><td>{reward.program}</td><td>{new Intl.NumberFormat('en-IN', { style: 'currency', currency: reward.currency || 'INR' }).format(Number(reward.amount))}</td><td>{reward.period}</td><td>{status === 'Rejected' ? 'Not announced' : <button className={styles.secondary} disabled={busy !== null || sent.includes(reward.id)} onClick={() => act(reward, status === 'Pending' ? 'approve' : 'notify')}>{busy === reward.id ? 'Sending…' : sent.includes(reward.id) ? 'Announced ✓' : status === 'Pending' ? 'Approve & announce' : 'Announce reward'}</button>}</td></tr>)}</tbody></table></div>
      </section>;
    })}
  </section>;
}
